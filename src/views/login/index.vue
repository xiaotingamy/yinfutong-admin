      ...
      <el-row :gutter="20">
        <el-col :span="18">
          <el-form-item prop="imgcaptcha">
            ...
            <el-input name="imgcaptcha" 
            type="text" 
            @keyup.enter.native="handleLogin" 
            v-model="loginForm.imgcaptcha" 
            autoComplete="off" 
            placeholder="captcha" 
            :maxlength="captchaMaxLength" ></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <img :src="imgCaptchaUrl" class="img-captcha"/>
        </el-col>
      </el-row>
...
// 先写死的图片验证码
import imgCaptchaUrl from '@/assets/404_images/404.png'
  ...
  data() {
    ...
    const validateCaptcha = (rule, value, callback) => {
      ...
      if (value.length < 4 || !reg.test(value)) {
        if (value.length === 0) {
          callback(new Error('The image captcha can not be empty'))
        } else {
          callback(new Error('please enter the correct image captcha'))
        }
      } else {
        callback()
      }
    ...
    return {
      ...
      loginRules: {
        ...
        imgcaptcha: [{ required: true, trigger: 'blur', validator: validateCaptcha }]
      },
